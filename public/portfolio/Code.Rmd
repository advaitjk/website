...
1. When judging the level of activity of Airbnb Beijing, we note how this platform still had room to grow as it isn't the giant we might assume.
...
1. When looking at the size of properties, we note how the median (and not average due to several outliers) properties has only **1 bedroom and 1 bathroom**. This indicates how typically, most rentals will be limited to one couple (and likely without kids), as confirmed by the median number of guests allowed (just 2 people). 
...
p12 + p13 + p14 + p15 +
  plot_annotation(title = "Apartments are the most common listing in Beijing", subtitle = "Over half of listings have a flexible cancellation policy, and 2/3rds list the entire property")
```
